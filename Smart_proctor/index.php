...
echo html_writer::start_div('flask-api-section', array('style' => 'margin: 20px 0; padding: 15px; border: 2px solid #ce09b7ff; border-radius: 8px;'));
echo html_writer::tag('h3', get_string('flask_api_title', 'local_cortex'), array('style' => 'color: #ce09b7ff; margin-top: 0;'));
...
if ($form_submitted && !empty($user_filename) && confirm_sesskey()) {
    ...
    if ($flask_result['success']) {
        ...
        if ($response_data['success']) {
            ...
            echo html_writer::tag('h4', '✅ ' . get_string('execution_success', 'local_cortex'), array('style' => 'color: #ce09b7ff; margin-top: 0;'));
        } else {
        ...
        if (!empty($response_data['stdout'])) {
            echo html_writer::tag('h5', get_string('program_output', 'local_cortex'), array('style' => 'color: #ce09b7ff; margin-top: 15px; margin-bottom: 5px;'));
            ...
            if (!empty($parsed_response)) {
                ...
                echo html_writer::tag('div', $parsed_response, 
                     array('style' => 'background-color: #f8f9fa; padding: 15px; border-radius: 5px; border-left: 4px solid #ce09b7ff; line-height: 1.6; white-space: pre-wrap;'));
            } else {
                // Fallback to raw output if parsing fails
                echo html_writer::tag('pre', htmlspecialchars($stdout_content), 
                     array('style' => 'background-color: #f8f9fa; padding: 10px; border-radius: 3px; border-left: 4px solid #ce09b7ff; font-family: monospace; white-space: pre-wrap;'));
            }
        }
...
echo html_writer::tag('h4', get_string('enter_script_info', 'local_cortex'), array('style' => 'color: #dc11a9ff;'));
...
echo html_writer::empty_tag('input', array(
    ...
    'style' => 'background: linear-gradient(45deg, #6a11cb 0%, #2575fc 100%); color: white; padding: 12px 24px; border: none; border-radius: 8px; cursor: pointer; font-size: 16px; font-weight: 600; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.2); transition: all 0.3s ease;'
));
